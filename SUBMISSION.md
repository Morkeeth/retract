...
on it, including the side effects that already executed. Concurrency control stops a bad
...
## Where the boundary is, stated before anyone has to find it

RETRACT reaches a side effect that already executed, decides a reversal is owed,
and records it exactly once. **It does not call a payment provider.** There is no
HTTP client, no SDK and no provider credential anywhere in this repository, and
`retract/compensate.py` says so at the line that writes the row.

What is real: every belief, contradiction, effect and reversal is a durable row
in CockroachDB Cloud, written in a serializable transaction, with the reversal
carrying its own idempotency key derived as `comp:<original>` so a replay
produces one reversal and not two. That is verifiable — query the cluster.

What is not: `status='executed'` on a reversal row is the LEDGER's lifecycle,
not a settlement. Every identifier on that row was generated by this process.
The row's payload carries `"dispatch": "none"` and `"external_receipt": null`
so the boundary survives being read straight out of the table.

The missing half is one adapter and its shape is already separable — record the
reversal as owed inside the transaction, dispatch outside it, record the
provider's identifier against the row. Until an identifier arrives that this
process did not generate, no claim here says the money moved.

We would rather name this precisely than blur it. A demo that says "the money
came back" is one query away from being caught; this one says what the query
will show.
