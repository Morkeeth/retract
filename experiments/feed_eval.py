...
ROWS = {
    SESSION: [{"subject": "Customer 4471", "predicate": "identity verified",
               "challenger": "Customer 4471 FAILED identity verification - passport is forged.",
               "challenger_by": "fraud-agent", "distance": 0.608,
               "detected_at": "2026-08-13T09:00:00Z"}],
    OTHER: [{"subject": "Customer 9902", "predicate": "address",
             "challenger": "9902 used to live in Berlin",
             "challenger_by": "support-agent", "distance": 0.296,
             "detected_at": "2026-08-13T09:01:00Z"}],
}
...
def fallback_contradictions(token: str, mcp_available: bool) -> dict:
    ...
    return {"available": True, "via": "cockroachdb-managed-mcp",
            "scope": grant.scope, "open": len(ROWS.get(grant.scope, []))}
...
def main() -> int:
    ...
    if not res.get("available") or res.get("open") != 1:
    ...
    if mine.get("open") != 1:
        ...
        ok = False
    elif theirs_via_forgery.get("available") is not False or theirs_via_forgery.get("open") is not None:
    ...
    if res.get("available") is not False or res.get("open") is not None:
    ...
    print(f"   {ctrl}")
    if ctrl.get("available") is not True or ctrl.get("open") is None:
